...
tasks.register<JacocoReport>("allDebugCoverage") {
    ...
    val jClasses: List<String> = subprojects.map { proj ->
        proj.layout.buildDirectory.dir("intermediates/javac/debug/classes").get().asFile.path
    }
    val kClasses: List<String> = subprojects.map { proj ->
        proj.layout.buildDirectory.dir("tmp/kotlin-classes/debug").get().asFile.path
    }
    ...
    val executions = subprojects.filter { proj ->
        val path = proj.layout.buildDirectory.file("jacoco/testDebugUnitTest.exec").get().asFile.path
        File(path).exists()
    }.map { proj ->
        proj.layout.buildDirectory.file("jacoco/testDebugUnitTest.exec").get().asFile.path
    }
